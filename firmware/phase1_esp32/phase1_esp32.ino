...
uint32_t uploadedVideoFrameCount = 0;
uint32_t droppedTalkAudioPackets = 0;
...
constexpr char PLACEHOLDER_WIFI_SSID[] = "your-ssid";
...
void publishStatus() {
  ...
  doc["streamEnabled"] = streamEnabled;
    ...
    switch (type) {
      ...
      case WStype_TEXT: {
        ...
        if (strcmp(messageType, "ctrl") == 0) {
          ...
          if (!talkEnabled) {
            droppedTalkAudioPackets = 0;
          }
        } else if (strcmp(messageType, "stream") == 0) {
          streamEnabled = doc["enabled"] | false;
          if (!streamEnabled) {
            safeStop();
          }
          Serial.printf("[STREAM] %s camera=%s latestSeq=%lu\n",
          ...
          if (!talkEnabled) {
            ...
            if ((droppedTalkAudioPackets % 10U) == 1U) {
              Serial.println("[SPK] talk_audio ignored because TALK is off");
            }
            return;
          }
          if (!speakerReady) {
            Serial.println("[SPK] talk_audio ignored because speaker is not ready");
            return;
          }
...
void ensureMediaInitialized() {
  ...
  Serial.println("[MEDIA] initializing");
  ...
  Serial.printf("[MEDIA] initialized camera=%s mic=%s speaker=%s\n",
    cameraReady ? "ok" : "fail",
    microphoneReady ? "ok" : "fail",
...
void handleControlJson() {
  ...
  if (doc.containsKey("stream")) {
    ...
    Serial.printf("[STREAM] %s (AP mode)\n", streamEnabled ? "enabled" : "disabled");
  }
...
void uploadVideoFrameIfNeeded() {
  ...
  if (sent) {
    ...
    if ((uploadedVideoFrameCount % 20U) == 0U) {
      Serial.printf("[CAM] uploaded frames=%lu latestSeq=%lu bytes=%u\n",
        static_cast<unsigned long>(uploadedVideoFrameCount),
        static_cast<unsigned long>(latestFrameSequence),
        static_cast<unsigned>(frameLen));
    }
  } else {
...
void setup() {
  ...
  const bool hasUsableBrokerConfig =
    hasStoredWifi &&
    activeWifiSsid.length() > 0 &&
    activeWifiSsid != PLACEHOLDER_WIFI_SSID &&
    activeBrokerHost.length() > 0 &&
    activeBrokerPort != 0;
  const bool bootToBroker = hasUsableBrokerConfig;
  if (bootToBroker) {
    ensureWifiConnected();
  } else {
    startProvisioningAp();
...
void loop() {
  ...
  if ((apMode || WiFi.isConnected()) && !mediaInitialized) {
    ensureMediaInitialized();
  }
